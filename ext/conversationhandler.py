...
class ConversationHandler(Handler):
    ...
    def find_handler(self, update, handlers):
        for candidate in (handlers or []):
            if candidate.check_update(update):
                handler = candidate
                return handler

    def find_final_handler(self, update, state):
        # Search entry points for a match
        if state is None or self.allow_reentry:
            handler = self.find_handler(update, self.entry_points)
            if handler is None and state is None:
                return

        # Get the handler list for current state, if we didn't find one yet and we're still here
        if state is not None and not handler:
            handlers = self.states.get(state)
            handler = self.find_handler(update, handlers)
            # Find a fallback handler if all other handlers fail
            if handler is None:
                handler = self.find_handler(update, self.fallbacks)
                if handler is None:
                    return
        return handler
    ...
    def check_update(self, update):
        ...
        state = self.conversations.get(key, None)
        ...
        if isinstance(state, tuple) and len(state) is 2 and isinstance(state[1], Promise):
            ...
            if new_state.done.is_set():
                ...
                state = old_state
                handlers = self.states.get(state)
                handler = self.find_final_handler(update, state)
                if handler is None:
                    return False
                self.process_handler = False
                return True
