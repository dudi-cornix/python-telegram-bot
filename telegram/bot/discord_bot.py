...
from telegram.utils.discord.request import Request

logging.getLogger(__name__).addHandler(logging.NullHandler())

COMMAND_PREFIX = "/"
async_bot = commands.Bot(command_prefix=COMMAND_PREFIX)
...
def signed_request(bot, url, method_type, data=None, **kwargs):
    headers = {"Authorization": f"{bot.token}"}
    return bot._request.request(method_type=method_type, url=url, headers=headers, data=data,
...
def message(func):
    @functools.wraps(func)
    def decorator(self, *args, **kwargs):
        url, data, method_type = func(self, *args, **kwargs)
        if kwargs.get('reply_to_message_id'):
            data["message_reference"] = {"message_id": kwargs.get('reply_to_message_id')}
        ...
        if kwargs.get('reply_markup'):
            ...
            if isinstance(reply_markup, ReplyMarkup):
                data['components'] = reply_markup.to_json(self.type)
            else:
                data['components'] = reply_markup

        result = signed_request(self, url, method_type, data=data, **kwargs)
...
class Bot(TelegramObject):
    def __init__(self, token, base_url=None, request=None):
        ...
        self.base_url = base_url or 'https://discord.com/api/v9'
        self.bot = None
        self.type = PlatformType.discord.value
        self._request = request or Request()
        self.logger = logging.getLogger(__name__)

    def run(self):
        async_bot.run(self.bot_token)
    ...
    def get_me(self, timeout=None, **kwargs):
        ...
        """
        url = '{0}/users/@me'.format(self.base_url)

        result = signed_request(self, url, "GET", timeout=timeout, **kwargs)
    ...
    def send_message(self,
                     ...
                     timeout=None,
                     **kwargs):
        url = '{0}/channels/{1}/messages'.format(self.base_url, chat_id)
    ...
    def delete_message(self, chat_id, message_id, timeout=None, **kwargs):
        url = '{0}/channels/{1}/messages/{2}'.format(self.base_url, chat_id, message_id)
        signed_request(self, url, "DELETE", timeout=timeout, **kwargs)
